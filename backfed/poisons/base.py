...
class Poison(ABC):
    ...
    def poison_test(self, net, test_loader, loss_fn=torch.nn.CrossEntropyLoss(), normalization=None):
        ...
        backdoor_accuracy = backdoored_preds / total_samples if total_samples > 0 else 0.0
        backdoor_loss = total_loss / len(test_loader) if len(test_loader) > 0 else 0.0
