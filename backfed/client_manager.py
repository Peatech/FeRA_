...
class ClientManager:
    ...
    def __init__(self, config, benign_client_class, malicious_client_class, start_round=0):
        ...
        self.cross_silo_mode = config.get("cross_silo", False)
        if self.cross_silo_mode:
                                               
            if self.config.num_clients_per_round != self.config.num_clients:
                log(WARNING, f"Cross-silo mode: num_clients_per_round ({self.config.num_clients_per_round}) != num_clients ({self.config.num_clients}). Setting num_clients_per_round to num_clients.")
                self.config.num_clients_per_round = self.config.num_clients
            
                                                     
            self.fixed_clients = list(range(self.config.num_clients))
            log(INFO, f"Cross-silo mode: Fixed clients = {self.fixed_clients}")
        else:
            self.fixed_clients = None
        ...
        if config.no_attack:
            ...
            if self.cross_silo_mode:
                self.fixed_benign_clients = self.benign_clients
                log(INFO, f"Cross-silo mode (no attack): Fixed benign clients = {self.fixed_benign_clients}")
            ...
            if self.cross_silo_mode:
                self.fixed_malicious_clients = self.malicious_clients
                self.fixed_benign_clients = self.benign_clients
                log(INFO, f"Cross-silo mode: Fixed malicious clients = {self.fixed_malicious_clients}, Fixed benign clients = {self.fixed_benign_clients}")
            ...
            if r not in self.poison_rounds:
                if self.cross_silo_mode:
                    ...
                    self.rounds_selection[r] = {
                        self.benign_client_class: self.fixed_benign_clients if hasattr(self, 'fixed_benign_clients') else self.benign_clients
                    }
                else:
    ...
    def _cross_silo_selection(self):
        """Cross-silo selection: All clients (benign + malicious) participate every round."""
        poison_frequency = self.atk_config.poison_frequency
        ...
        self.poison_rounds = selected_poison_rounds
        
                                                                 
                                                                        
        for r in selected_poison_rounds:
            self.rounds_selection[r] = {
                self.malicious_client_class: self.fixed_malicious_clients,
                self.benign_client_class: self.fixed_benign_clients
            }
        
        log(INFO, f"Cross-silo selection: All {self.config.num_clients} clients participate in poison rounds {selected_poison_rounds}")
        log(INFO, f"  Malicious clients: {self.fixed_malicious_clients}, Benign clients: {self.fixed_benign_clients}")
                    ...
                    if len(round_sample_space) > 0:
                        ...
                        if replaced_client in self.malicious_clients:
                            if self.malicious_client_class not in self.rounds_selection[r]:
                                self.rounds_selection[r][self.malicious_client_class] = [replaced_client]                               
                            else:
                            ...
                            if self.benign_client_class not in self.rounds_selection[r]:
                                self.rounds_selection[r][self.benign_client_class] = [replaced_client]                               
                            else:
    ...
    def visualize_client_selection(self, start_round=-1, end_round=-1, interval=2, only_poison_rounds=True, save_path=None):
        ...
        if start_round == -1:
            start_round = max(1, end_round - 100)                                               
        ...
        for idx, r in enumerate(selected_rounds):
            if r in self.rounds_selection:
                ...
                data[selected_clients, idx] = 1                              

        fig_width = num_rounds                                            
        fig_height = num_clients                                           
        ...
        poisoned_rounds = self.get_poison_rounds()                                   
