...
class AnomalyDetectionServer(UnweightedFedAvgServer):
    ...
    def gap_statistics(self, data: np.ndarray, num_sampling: int, K_max: int, n: int) -> int:
        ...
        for k in range(1, K_max + 1):
            ...
            for _ in range(num_sampling):
                ...
                wkbs.append(np.log(wkb + 1e-10))                                     

                                     
            gap = np.mean(wkbs) - np.log(wk + 1e-10)                                     
    ...
    def evaluate_detection(self, malicious_clients: List[int], true_malicious_clients: List[int], total_updates: int):
        ...
        verbose_det = getattr(self, "verbose_detection_logging", True)
        ...
        if verbose_det:
            ...
            if len(true_malicious_clients) == 0:
                log(INFO, f"[Round {self.current_round}] {self.__class__.__name__} flagged={sorted(detected_set)} fpr_clean={fpr:.3f}")
            else:
                log(INFO, f"[Round {self.current_round}] {self.__class__.__name__} flagged={sorted(detected_set)} precision={precision:.3f} recall={recall:.3f} fpr={fpr:.3f}")

        if self.current_round == self.start_round + self.config.num_rounds:
            ...
            if verbose_det:
                ...
                log(INFO, f"[Round {self.current_round}] {self.__class__.__name__} summary={summary_detection_metrics}")
