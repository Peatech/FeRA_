...
class MaliciousClient(BaseClient):
    ...
    def _update_and_sync_poison(self, selected_malicious_clients, server_round, normalization=None):
        ...
        selected_client = selected_malicious_clients[0]                                                     
    ...
    def sync_poison_module(self, selected_client, server_round):
        if self.client_id == selected_client:
            resource_package = self.poison_module.get_shared_resources()                             
    ...
    def train(self, train_package):
        ...
        if self.poison_module.sync_poison:                                            
    ...
    def evaluate(self, test_package: Dict[str, Any]) -> Tuple[int, Metrics]:
        ...
        val_backdoor_samples, val_backdoor_loss, val_backdoor_accuracy = self.poison_module.poison_test(self.model, self.val_loader, normalization=self.normalization)
        metrics.update({
            'val_backdoor_samples': val_backdoor_samples,
