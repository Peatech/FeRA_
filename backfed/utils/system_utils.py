...
def system_startup(config: DictConfig):
    ...
    config.client_config.optimizer = config.client_optimizer_config[config.client_config.optimizer]                                                  
    config.atk_config.atk_optimizer = config.atk_config.atk_optimizer_config[config.atk_config.atk_optimizer]                                               
    
                               
    if config.get("cross_silo", False):
                                           
        if not hasattr(config, "cross_silo_num_clients") or config.cross_silo_num_clients <= 0:
            raise ValueError("cross_silo_num_clients must be set and > 0 when cross_silo=true")
        if not hasattr(config, "cross_silo_num_attackers") or config.cross_silo_num_attackers < 0:
            raise ValueError("cross_silo_num_attackers must be set and >= 0 when cross_silo=true")
        if config.cross_silo_num_attackers >= config.cross_silo_num_clients:
            raise ValueError(f"cross_silo_num_attackers ({config.cross_silo_num_attackers}) must be < cross_silo_num_clients ({config.cross_silo_num_clients})")
        
                                                                       
        config.num_clients = config.cross_silo_num_clients
        config.num_clients_per_round = config.cross_silo_num_clients                                       
        log(INFO, f"Cross-silo mode enabled: num_clients={config.num_clients}, num_clients_per_round={config.num_clients_per_round}, num_attackers={config.cross_silo_num_attackers}")
...
def set_attack_config(config: DictConfig):
    ...
    if config.get("cross_silo", False):
        num_attackers = config.cross_silo_num_attackers
                                                                                  
        config.atk_config.malicious_clients = list(range(num_attackers))
        log(INFO, f"Cross-silo mode: Selected {num_attackers} attackers: {config.atk_config.malicious_clients}")
    else:
        ...
        if config.atk_config.adversary_selection == "random":
            ...
            if not config.atk_config.malicious_clients:
                config.atk_config.malicious_clients = [0]                       
